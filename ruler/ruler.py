...
class APP (Frame):
    ...
    def createWidgets(self):
        ...
        self.PreviewCanvas = Canvas(
            self.PreviewFrame, width=500, height=300, bg='white', bd='3', relief='raised')
        self.PreviewCanvas.grid(sticky=N+S+E+W)
        self.PreviewCanvas.config(scrollregion=(0, 0, 10000, 300))
        self.PreviewScroll = Scrollbar(
            self.PreviewFrame, command=self.PreviewCanvas.xview, orient=HORIZONTAL)
        ...
        self.XLine = self.PreviewCanvas.create_line(
            15, 150, 65, 150, fill='red')
        self.YLine = self.PreviewCanvas.create_line(
            15, 160, 15, 100, fill='green')
        ...
        self.UnitIn = Radiobutton(self.UnitFrame, text='Inch', value=1,
                                  variable=self.UnitVar, indicatoron=0, width=6, command=self.UnitSelect)
        self.UnitIn.grid(row=1, column=0)
        self.UnitMM = Radiobutton(self.UnitFrame, text='MM', value=2,
                                  variable=self.UnitVar, indicatoron=0, width=6, command=self.UnitSelect)
        ...
        self.Preamble = Entry(
            self.EntryFrame, textvariable=self.PreambleVar, width=15)
        ...
        self.RulerLength = Entry(
            self.EntryFrame, textvariable=self.RulerLengthVar, width=15)
        ...
        self.MajorStripeLength = Entry(
            self.EntryFrame, textvariable=self.MajorStripeLengthVar, width=15)
        ...
        self.HalfStripeLength = Entry(
            self.EntryFrame, textvariable=self.HalfStripeLengthVar, width=15)
        ...
        self.MinorStripeLength = Entry(
            self.EntryFrame, textvariable=self.MinorStripeLengthVar, width=15)
        ...
        self.StartX = Entry(
            self.EntryFrame, textvariable=self.StartXVar, width=15)
        ...
        self.StartY = Entry(
            self.EntryFrame, textvariable=self.StartYVar, width=15)
        ...
        self.RulerStripesEvery = Entry(
            self.EntryFrame, textvariable=self.RulerStripesEveryVar, width=15)
        ...
        self.MajorStripeEvery = Entry(
            self.EntryFrame, textvariable=self.MajorStripeEveryVar, width=15)
        ...
        self.Font = OptionMenu(self.EntryFrame, self.FontVar,
                               *fontList, command=self.ChooseFont)
        ...
        self.FontXOffset = Entry(
            self.OffsetFrame, textvariable=self.FontXOffsetVar, width=5)
        ...
        self.FontYOffset = Entry(
            self.OffsetFrame, textvariable=self.FontYOffsetVar, width=5)
        ...
        self.XScale = Entry(
            self.ScaleFrame, textvariable=self.XScaleVar, width=5)
        ...
        self.YScale = Entry(
            self.ScaleFrame, textvariable=self.YScaleVar, width=5)
    ...
    def DoIt(self):
        ...
        try:
            ...
            fontFileName = self.FontVar.get()
            file = open(fontPath+fontFileName, encoding="Latin1")
        except:
        ...
        # nominal inches(mm) / pixel for plotting
        Scale = MajorSL * 2.0 * 1.2 / 200.0
        Angle = 0.0
        ...
        self.gcode.append(
            '( Code generated by ruler-%s.py widget )' % (version))
        self.gcode.append('( by Andrew Williams - 2018 )')
        self.gcode.append('( Engraving a %d tick, %.3f %s long ruler )' % (
            NumTicks, Length, UnitS))
        ...
        self.gcode.append(
            "( [#1 = 0 or 1 for a G0 or G1 type of move], [#2=x], [#3=y], [#4=realx], [#5=realy])")
        ...
        for tick in range(0, NumTicks):
            ...
            if ((tick % MEvery) == 0 and tick != 0):  # Write the number for this major line
                ...
                for char in String:
                    ...
                    try:
                        ...
                        for stroke in font[char].stroke_list:
                            ...
                            if (dist > 0.001) or first_stroke:
                                ...
                                self.gcode.append("G0 Z#1001")
                                self.gcode.append(
                                    'o9000 call [0] [%.4f] [%.4f] [%.4f] [%.4f]' % (x1, y1, RealX, RealY))
                                self.gcode.append("G1 Z#1002")

                            x2 = stroke.xend + xoffset
                            y2 = stroke.yend
                            self.gcode.append(
                                'o9000 call [1] [%.4f] [%.4f] [%.4f] [%.4f]' % (x2, y2, RealX, RealY))
                            ...
                            self.segID.append(self.PreviewCanvas.create_line(
                                15+(x1+RealX)/Scale, 150-(y1+RealY)/Scale, 15 +
                                (x2+RealX)/Scale, 150-(y2+RealY)/Scale,
                                fill='black', width=1))
                    ...
                    # raise engraver
                    self.gcode.append('G0 Z#1001')
                    # blank line after every char block
                    self.gcode.append("")
