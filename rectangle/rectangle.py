...
# python rectangle.py
# December 1, 2020
# Rectangle G-Code Generator for LinuxCNC
"""
    ...
    the command python YourPathToThisFile/rectangle.py
...
class APP (Frame):
    ...
    def createMenu(self):
        ...
        self.EditMenu.add_command(
            label='Select All', command=self.SelectAllText)
        self.EditMenu.add_command(
            label='Delete All', command=self.ClearTextBox)
        self.EditMenu.add_separator()
        self.EditMenu.add_command(
            label='Save Preferences', command=self.SavePrefs)
        self.EditMenu.add_command(
            label='Load Preferences', command=self.LoadPrefs)
    ...
    def createWidgets(self):
        ...
        self.st1 = Label(self, text='Size X *')
        self.st1.grid(row=1, column=0, sticky=E)
        self.sizeXVar = StringVar()
        self.partLength = Entry(self, width=10, textvariable=self.sizeXVar)
        self.partLength.grid(row=1, column=1, sticky=W)
        self.partLength.focus_set()

        self.st2 = Label(self, text='Size Y *')
        self.st2.grid(row=2, column=0, sticky=E)
        self.sizeYVar = StringVar()
        self.sizeY = Entry(self, width=10, textvariable=self.sizeYVar)
        self.sizeY.grid(row=2, column=1, sticky=W)
        self.sizeY.focus_set()

        self.st6 = Label(self, text='Depth of Each Cut ')
        self.st6.grid(row=3, column=0, sticky=E)
        self.depthOfCutVar = StringVar()
        self.depthOfCut = Entry(
            self, width=10, textvariable=self.depthOfCutVar)
        self.depthOfCut.grid(row=3, column=1, sticky=W)

        self.st5 = Label(self, text='Total Depth *')
        self.st5.grid(row=4, column=0, sticky=E)
        self.TotalDepthVar = StringVar()
        self.TotalDepth = Entry(
            self, width=10, textvariable=self.TotalDepthVar)
        self.TotalDepth.grid(row=4, column=1, sticky=W)
        ...
        InOutOptions = [('Inside', 0), ('Outside', 1)]
        self.InOutVar = IntVar()
        for text, value in InOutOptions:
            if (value == 0):
                Radiobutton(self, text=text, value=value,
                            variable=self.InOutVar, indicatoron=0, width=6,)\
                    .grid(row=5, column=value+2, sticky=E)
            else:
                Radiobutton(self, text=text, value=value,
                            variable=self.InOutVar, indicatoron=0, width=6,)\
                    .grid(row=5, column=value+2, sticky=W)
        ...
        self.ToolDiameter = Entry(
            self, width=10, textvariable=self.ToolDiameterVar)
        ...
        self.Feedrate = Entry(
            self, width=10, textvariable=self.FeedrateVar)
        ...
        self.SpindleRPM = Entry(
            self, width=10, textvariable=self.SpindleRPMVar)
        ...
        self.GenButton = Button(
            self, text='Generate G-Code', command=self.GenCode)
        self.GenButton.grid(row=8, column=0)

        self.CopyButton = Button(
            self, text='Select All & Copy', command=self.SelectCopy)
        self.CopyButton.grid(row=8, column=1)

        self.WriteButton = Button(
            self, text='Write to File', command=self.WriteToFile)
        ...
        if IN_AXIS:
            ...
            self.quitButton = Button(
                self, text='Quit', command=self.QuitFromAxis)
    ...
    def QuitFromAxis(self):
        sys.stdout.write("M2 (Rectangle.py Aborted)")
        self.quit()
    ...
    def GenCode(self):
        """ Generate the G-Code for cutting a rectangle in a part 
        assume that the part is at X0 to X+, Y0 to Y-"""
        D = Decimal

        self.ToolRadius = self.FToD(self.ToolDiameterVar.get())/2

        self.sizeX = self.FToD(self.sizeXVar.get())
        self.sizeY = self.FToD(self.sizeYVar.get())
        ...
        if (self.InOutVar.get() == 0):
            # Cutting inside the defined rectangle
            #
            # Calculate the corner positions.   The corners are numbered 0-3, working counter-clockwise
            # from the starting position
            if self.HomeVar.get() == 4:       # origin right rear
                self.X_Start = -(self.LeadIn) - D('0.1')
                self.Y_Start = -(self.LeadIn) - D('0.1')

                self.Corner0_X = -(self.LeadIn)
                self.Corner0_Y = -(self.LeadIn)

                self.Corner1_X = -self.sizeX + self.ToolRadius
                self.Corner1_Y = self.Corner0_Y

                self.Corner2_X = self.Corner1_X
                self.Corner2_Y = -self.sizeY + self.ToolRadius

                self.Corner3_X = self.Corner0_X
                self.Corner3_Y = self.Corner2_Y
            else:                           # origin left front
                self.X_Start = (self.LeadIn) + D('0.1')
                self.Y_Start = (self.LeadIn) + D('0.1')
            ...
            # Cutting outside the defined rectangle
            #
            # Calculate the corner positions.   The corners are numbered 0-3, working clockwise
            # from the starting position
            ...
            if self.HomeVar.get() == 4:       # origin right rear
                ...
                self.Corner1_X = self.Corner0_X
                self.Corner1_Y = -self.sizeY - self.ToolRadius

                self.Corner2_X = -self.sizeX - self.ToolRadius
                self.Corner2_Y = self.Corner1_Y

                self.Corner3_X = self.Corner2_X
                self.Corner3_Y = self.Corner0_Y
            else:
                self.X_Start = -(self.LeadIn) - D('0.1')
                self.Y_Start = -(self.LeadIn) - D('0.1')
        ...
        self.Z_Total = self.FToD(self.TotalDepthVar.get())
        if len(self.depthOfCutVar.get()) > 0:
            self.Z_Step = self.FToD(self.depthOfCutVar.get())
            self.NumOfZSteps = int(
                self.FToD(self.TotalDepthVar.get()) / self.Z_Step)
        ...
        for i in range(self.NumOfZSteps):
            ...
            self.g_code.insert(END, 'G1 X%.4f Y%.4f\n' %
                               (self.Corner0_X, self.Corner0_Y))
            self.g_code.insert(END, 'G1 X%.4f Y%.4f\n' %
                               (self.Corner1_X, self.Corner1_Y))
            self.g_code.insert(END, 'G1 X%.4f Y%.4f\n' %
                               (self.Corner2_X, self.Corner2_Y))
            self.g_code.insert(END, 'G1 X%.4f Y%.4f\n' %
                               (self.Corner3_X, self.Corner3_Y))
            self.g_code.insert(END, 'G1 X%.4f Y%.4f\n' %
                               (self.Corner0_X, self.Corner0_Y))
    ...
    def GetDirectory(self):
        self.DirName = askdirectory(
            initialdir='/home', title='Please select a directory')
    ...
    def LoadPrefs(self):
        self.NcDir = self.GetIniData(
            'rectangle.ini', 'Directories', 'NcFiles', os.path.expanduser("~"))
        self.FeedrateVar.set(self.GetIniData(
            'rectangle.ini', 'MillingPara', 'Feedrate', '1000'))
        self.depthOfCutVar.set(self.GetIniData(
            'rectangle.ini', 'MillingPara', 'depthOfCut', '3'))
        self.ToolDiameterVar.set(self.GetIniData(
            'rectangle.ini', 'MillingPara', 'ToolDiameter', '10'))
        self.SpindleRPMVar.set(self.GetIniData(
            'rectangle.ini', 'MillingPara', 'SpindleRPM', '9000'))
        self.LeadinVar.set(self.GetIniData(
            'rectangle.ini', 'MillingPara', 'Leadin'))
        self.UnitVar.set(
            int(self.GetIniData('rectangle.ini', 'MillingPara', 'UnitVar', '2')))
        self.HomeVar.set(
            int(self.GetIniData('rectangle.ini', 'MillingPara', 'HomeVar', '4')))
        self.InOutVar.set(
            int(self.GetIniData('rectangle.ini', 'MillingPara', 'InOutVar', '1')))
        self.SafeZVar.set(self.GetIniData(
            'rectangle.ini', 'MillingPara', 'SafeZ', '10.0'))
        self.sizeXVar.set(self.GetIniData(
            'rectangle.ini', 'Part', 'Size X', '10'))
        self.sizeYVar.set(self.GetIniData(
            'rectangle.ini', 'Part', 'Size Y', '10'))
        self.TotalDepthVar.set(self.GetIniData(
            'rectangle.ini', 'Part', 'TotalDepth'))
    ...
    def SavePrefs(self):
        ...
        set_pref('MillingPara', 'InOutVar', self.InOutVar.get())
        set_pref('MillingPara', 'SafeZ', self.SafeZVar.get())
        set_pref('Part', 'X Size', self.sizeXVar.get())
        set_pref('Part', 'Y Size', self.sizeYVar.get())
        set_pref('Part', 'TotalDepth', self.TotalDepthVar.get())
        self.cp.write(self.fn)
        self.fn.close()

    def Simple(self):
        messagebox.showinfo(
            'Feature', 'Sorry this Feature has\nnot been programmed yet.')
...
root = tk.Tk()
app = APP(master=root)
app.master.title('Rectangle Cutting G-Code Generator - V' + version)
app.mainloop()
