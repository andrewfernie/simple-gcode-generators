...
"""
    ...
    Version 1.1.0 - 2025-01-14 - Andrew Fernie
...
class APP (Frame):
    ...
    def createWidgets(self):
        ...
        Radiobutton(self.EntryFrame, text='Circle', variable=self.Shape, value="Circel",
                    command=self.DoIt, indicatoron=0, width=5,).grid(row=self.rownumber+1, column=0, sticky=N)
        Radiobutton(self.EntryFrame, text=' Rect ', variable=self.Shape, value="Rectangular",
                    command=self.DoIt, indicatoron=0, width=5,).grid(row=self.rownumber+2, column=0, sticky=N)
        ...
        Radiobutton(self.EntryFrame, text='YES', variable=self.OutlineVar, value="yes", command=self.DoIt,
                    indicatoron=0, width=5,).grid(row=self.rownumber+1, column=1, sticky=W, padx=6)
        Radiobutton(self.EntryFrame, text='No', variable=self.OutlineVar, value="no", command=self.DoIt,
                    indicatoron=0, width=5,).grid(row=self.rownumber+2, column=1, sticky=W, padx=6)
        ...
        Radiobutton(self.EntryFrame, text='  Symetric', variable=self.grid_style, value="symetric",
                    command=self.DoIt, indicatoron=0, width=10,).grid(row=self.rownumber+1, column=1, sticky=N, padx=35)
        Radiobutton(self.EntryFrame, text='Line Count', variable=self.grid_style, value="count",
                    command=self.DoIt, indicatoron=0, width=10,).grid(row=self.rownumber+2, column=1, sticky=N, padx=15)
        ...
        Radiobutton(self.EntryFrame, text='Inch', variable=self.UnitVar, value=0, command=self.Change_Units,
                    indicatoron=0, width=5,).grid(row=self.rownumber+1, column=0, sticky=W)
        Radiobutton(self.EntryFrame, text=' MM ', variable=self.UnitVar, value=1, command=self.Change_Units,
                    indicatoron=0, width=5,).grid(row=self.rownumber+2, column=0, sticky=W)
        ...
        self.Cross.set("no")
        Radiobutton(self.EntryFrame, text='NO', variable=self.Cross, value="no", command=self.DoIt,
                    indicatoron=0, width=5,).grid(row=self.rownumber+1, column=0, sticky=E)
        Radiobutton(self.EntryFrame, text='YES', variable=self.Cross, value="yes", command=self.DoIt,
                    indicatoron=0, width=5,).grid(row=self.rownumber+2, column=0, sticky=E)
        self.grid_onVar = StringVar()
        self.grid_onVar.set("yes")
        self.label_grid_on = Label(self.EntryFrame, text='Grid ON', width=7)
        self.label_grid_on.grid(row=self.rownumber+1,
                                column=1, sticky=W, padx=60)
        Checkbutton(self.EntryFrame, text="", variable=self.grid_onVar, onvalue="yes",
                    offvalue="no", command=self.DoIt).grid(row=self.rownumber+2, column=1, sticky=W, padx=70)

        self.label_parallel = Label(
            self.EntryFrame, text='Parallel Outline', width=11)
        self.label_parallel.grid(row=self.rownumber, column=1, sticky=E)
        self.grid_parallelVar = StringVar()
        self.grid_parallelVar.set("no")
        Radiobutton(self.EntryFrame, text='YES', variable=self.grid_parallelVar, value="yes",
                    command=self.DoIt, indicatoron=0, width=4,).grid(row=self.rownumber+1, column=1, sticky=E, padx=15)
        Radiobutton(self.EntryFrame, text='No', variable=self.grid_parallelVar, value="no", command=self.DoIt,
                    indicatoron=0, width=4,).grid(row=self.rownumber+2, column=1, sticky=E, padx=15)
        ...
        self.Preamble = Entry(
            self.EntryFrame, textvariable=self.PreambleVar, width=35)
        ...
        self.XPocketCenter = Entry(
            self.EntryFrame, textvariable=self.XPocketCenterVar, width=10)
        ...
        self.YPocketCenter = Entry(
            self.EntryFrame, textvariable=self.YPocketCenterVar, width=10)
        ...
        self.PocketX = Entry(
            self.EntryFrame, textvariable=self.PocketXVar, width=15)
        self.PocketX.grid(row=self.rownumber, column=1, sticky=W)
        self.PocketYVar = StringVar()
        self.PocketYVar.set('30.0')
        self.PocketY = Entry(
            self.EntryFrame, textvariable=self.PocketYVar, width=15)
        ...
        self.grid_lines_X = Entry(
            self.EntryFrame, textvariable=self.grid_lines_X_var, width=5)
        self.grid_lines_X.grid(row=self.rownumber, column=1, sticky=W)
        self.grid_lines_Y_var = StringVar()
        self.grid_lines_Y_var.set('5')
        self.grid_lines_Y = Entry(
            self.EntryFrame, textvariable=self.grid_lines_Y_var, width=5)
        ...
        self.Stepover = Entry(
            self.EntryFrame, textvariable=self.StepoverVar, width=5)
        self.Stepover.grid(row=self.rownumber, column=1, sticky=W)
        Label(self.EntryFrame, text=' Stepover/Unit').grid(row=self.rownumber,
                                                           column=1, sticky=W, padx=35)

        self.rownumber += 1
        self.grid_border = IntVar()
        self.grid_border.set(1)
        Checkbutton(self.EntryFrame, text='Grid Border +/-', variable=self.grid_border,
                    onvalue=1, offvalue=0, command=self.DoIt).grid(row=self.rownumber, column=0, sticky=W)
        self.borderVar = StringVar()
        self.borderVar.set('2.0')
        self.border = Entry(
            self.EntryFrame, textvariable=self.borderVar, width=5)
        self.border.grid(row=self.rownumber, column=1, sticky=W)
        # cross
        self.cross_offset_label = Label(
            self.EntryFrame, text='Cross Overlab +/-')
        self.cross_offset_label.grid(
            row=self.rownumber, column=1, sticky=E, padx=55)
        self.cross_offset = StringVar()
        self.cross_offset.set('2.0')
        self.cross_offset_entry = Entry(
            self.EntryFrame, textvariable=self.cross_offset, width=5)
        ...
        Checkbutton(self.EntryFrame, text='Spider Grid', variable=self.spider_grid, onvalue="yes",
                    offvalue="no", command=self.DoIt).grid(row=self.rownumber, column=0, sticky=W)
        self.spider_lines_entry_label = Label(self.EntryFrame, text='Lines').grid(
            row=self.rownumber, column=0, sticky=E, padx=1)
        self.spider_lines = StringVar()
        self.spider_lines.set('4')
        self.spider_lines_entry = Entry(
            self.EntryFrame, textvariable=self.spider_lines, width=3)
        self.spider_lines_entry.grid(
            row=self.rownumber, column=1, sticky=W, padx=2)
        self.spider_startangel_entry_label = Label(self.EntryFrame, text='StartAngel').grid(
            row=self.rownumber, column=1, sticky=E, padx=35)
        self.spider_startangel = StringVar()
        self.spider_startangel.set('45')
        self.spider_startangel_entry = Entry(
            self.EntryFrame, textvariable=self.spider_startangel, width=3)
        self.spider_startangel_entry.grid(
            row=self.rownumber, column=1, sticky=E, padx=4)
        self.spider_offset_entry_label = Label(
            self.EntryFrame, text='SpiderOffset+/-').grid(row=self.rownumber, column=1, sticky=W, padx=35)
        self.spider_offset = StringVar()
        self.spider_offset.set('2.0')
        self.spider_offset_entry = Entry(
            self.EntryFrame, textvariable=self.spider_offset, width=5)
        self.spider_offset_entry.grid(
            row=self.rownumber, column=1, sticky=E, padx=105)
        ...
        self.HoleDepth = Entry(
            self.EntryFrame, textvariable=self.FinalDepth, width=8)
        ...
        self.SafeZVar.set('3')
        self.SafeZ = Entry(self.EntryFrame, width=8,
        ...
        self.RapidZ = Entry(self.EntryFrame, width=8,
                            textvariable=self.RapidZVar)
        ...
        self.Feedspeed = Entry(
            self.EntryFrame, textvariable=self.FeedspeedVar, width=8)
        ...
        self.FeedspeedZ = Entry(
            self.EntryFrame, textvariable=self.FeedspeedZVar, width=8)
        ...
        self.Postamble = Entry(
            self.EntryFrame, textvariable=self.PostambleVar, width=35)
    ...
    def DoIt(self):
        ...
        self.gcode.append('G0 Z%.3f ' %
                          (float(self.SafeZVar.get())))  # G0 Zsafe
        self.gcode.append('G0 X%.3f Y%.3f ' % (float(self.XPocketCenterVar.get()), float(
            self.YPocketCenterVar.get())))  # G0 CenterZsafe
        # get the inputs
        ...
        if self.Shape.get() == "Circel":      # circel
            ...
            if self.OutlineVar.get() == "yes":
                ...
                self.Go_down()
                self.gcode.append('G03 I%.3f F%.2f' % (
                    (float(self.PocketXVar.get())/2.0), float(self.FeedspeedVar.get())))
                self.Go_up()
                self.CanvasDrawings.append(self.PreviewCanvas.create_oval(
            ...
            if self.Cross.get() == "yes":
                newx = ((float(self.PocketXVar.get())/2.0) +
                        float(self.cross_offset_entry.get()))/Scale
                self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                    150, 150-newx, 150, 150+newx, fill="red"))
                self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                    150-newx, 150, 150+newx, 150, fill="red"))
                self.gcode.append('(Cross)')
                self.gcode.append('G0 X%.3f Y%.3f' %
                                  (centerx, (centery-pocket_rad-cross_offset)))
                self.Go_down()
                self.gcode.append('G1 Y%.3f F%.2f' % (
                    (centery+pocket_rad+cross_offset), float(self.FeedspeedVar.get())))
                self.Go_up()
                self.gcode.append('G0 X%.3f Y%.3f' %
                                  (centerx-pocket_rad-cross_offset, centery))
                self.Go_down()
                self.gcode.append('G1 X%.3f F%.2f' % (
                    (centerx+pocket_rad+cross_offset), float(self.FeedspeedVar.get())))
                self.Go_up()
            # circel grid
            if self.grid_onVar.get() == "yes":
                ...
                for line in range(1, lines+1):
                    ...
                    self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                        newx, 150-newy, newx, 150+newy, fill='red'))
                    if self.grid_style.get() == "symetric":
                        self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                            150-newy, newx, 150+newy, newx, fill='red'))
                        gcode_y = line * \
                            float(pocket_diameter/(lines+1))+centery-pocket_rad
                        self.gcode.append('G0 X%.3f Y%.3f' %
                                          (centerx-(newy*Scale), gcode_y))
                        self.Go_down()
                        self.gcode.append('G1 X%.3f F%.2f' % (
                            centerx+(newy*Scale), float(self.FeedspeedVar.get())))
                        self.Go_up()
                    # gcode
                    gcode_x = line*float(pocket_diameter /
                                         (lines+1))+centerx-pocket_rad
                    self.gcode.append('G0 X%.3f Y%.3f' %
                                      (gcode_x, centery-(newy*Scale)))
                    self.Go_down()
                    self.gcode.append('G1 Y%.3f F%.2f' % (
                        centery+(newy*Scale), float(self.FeedspeedVar.get())))
                    self.Go_up()
            ...
            if self.grid_parallelVar.get() == "yes":
                ...
                for circle in range(1, circels):
                    self.gcode.append('G0 X%.3f Y%.3f' %
                                      (centerx-(circle*space_x), centery))
                    self.Go_down()
                    self.gcode.append('G03 I%.3f F%.2f' % (
                        (circle*space_x), float(self.FeedspeedVar.get())))
                    self.Go_up()
                    self.CanvasDrawings.append(self.PreviewCanvas.create_oval(
            ...
            if self.spider_grid.get() == "yes":
                ...
                for line in range(angel_lines):
                    newx = sin(radians((line*angel_space)+start_angel)
                               )*(pocket_rad+spider_offset)
                    newy = cos(radians((line*angel_space)+start_angel)
                               )*(pocket_rad+spider_offset)
                    self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                        150, 150, 150-newx/Scale, 150-newy/Scale, fill="red"))
                    self.gcode.append('G0 X%.3f Y%.3f' % (centerx, centery))
                    self.Go_down()
                    self.gcode.append('G1 X%.3f Y%.3f F%.2f' % (
                        centerx-newx, centery-newy, float(self.FeedspeedVar.get())))
                    self.Go_up()
        # --------------
        # RECTANGULAR
        # --------------
        if self.Shape.get() == "Rectangular":      # rectangle
            ...
            if self.OutlineVar.get() == "yes":
                ...
                ))-(float(self.PocketXVar.get())/2.0), (float(self.YPocketCenterVar.get())-(float(self.PocketYVar.get())/2.0))))
                self.gcode.append('G1 Z%.3f F%.2f' % (
                    float(self.FinalDepth.get()), float(self.FeedspeedZVar.get())))
                self.gcode.append('G1 X%.3f F%.2f' % (float(self.XPocketCenterVar.get(
                ))+(float(self.PocketXVar.get())/2.0), float(self.FeedspeedVar.get())))
                self.gcode.append('G1 Y%.3f' % (
                    float(self.YPocketCenterVar.get())+(float(self.PocketYVar.get())/2.0)))
                self.gcode.append('G1 X%.3f' % (
                    float(self.XPocketCenterVar.get())-(float(self.PocketXVar.get())/2.0)))
                self.gcode.append('G1 Y%.3f' % (
                    float(self.YPocketCenterVar.get())-(float(self.PocketYVar.get())/2.0)))
            ...
            if self.grid_parallelVar.get() == "yes":
                ...
                for rec in range(1, recs):
                    ...
                    self.Go_down()
                    self.gcode.append('G1 X%.3f F%.2f' % (
                        centerx-newx, float(self.Feedspeed.get())))
            ...
            if self.Cross.get() == "yes":
                ...
                self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                    150, 150-newy, 150, 150+newy, fill="red"))
                self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                    150-newx, 150, 150+newx, 150, fill="red"))
                self.gcode.append('(Cross)')
                # g-code Y cross
                ...
                self.Go_down()
                self.gcode.append('G1 Y%.3f F%.2f' % ((float(self.YPocketCenterVar.get(
                ))+(float(self.PocketYVar.get())/2.0)+float(self.cross_offset_entry.get())), float(self.FeedspeedVar.get())))
                self.Go_up()
                # g-code x cross
                ...
                self.Go_down()
                self.gcode.append('G1 X%.3f F%.2f' % ((float(self.XPocketCenterVar.get(
                ))+(float(self.PocketXVar.get())/2.0)+float(self.cross_offset_entry.get())), float(self.FeedspeedVar.get())))
                self.Go_up()
            ...
            if self.grid_onVar.get() == "yes":
                ...
                if self.grid_style.get() == "symetric":
                    lines_x = int(float(self.PocketX.get()) /
                                  float(self.Stepover.get()))
                    lines_y = int(float(self.PocketY.get()) /
                                  float(self.Stepover.get()))
                    space_x = float(self.PocketX.get())/(lines_x+1)/Scale
                    space_y = float(self.PocketY.get())/(lines_y+1)/Scale
                ...
                for line in range(1, lines_y+1):
                    ...
                    if self.grid_border.get() == 1:
                        ...
                        newx = a/Scale
                    self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                        150-newx, newy, 150+newx, newy, fill='red'))
                # gcode
                self.gcode.append('(xGrid)')
                for line in range(1, lines_x+1):
                    newx = (float(self.XPocketCenter.get())-(float(self.PocketX.get())/2.0)
                            ) + line*(float(self.PocketX.get())/(lines_x+1))
                    if self.grid_border.get() == 1:
                        newy_center_offset = (
                            float(self.PocketY.get())/2.0) - float(self.border.get())
                        y_mill_length = float(
                            self.PocketY.get())-(2*float(self.border.get()))
                    else:
                        newy_center_offset = float(self.PocketY.get())/2.0
                        y_mill_length = float(self.PocketY.get())
                    # zig zag
                    if line % 2 == 0:
                        newy = float(self.YPocketCenter.get()) + \
                            newy_center_offset
                        ...
                        self.Go_down()
                        self.gcode.append('G1 G91 Y+%.3f' % y_mill_length)
                    self.gcode.append('G0 G90 Z%.3f' %
                                      (float(self.SafeZVar.get())))
                # """
                self.gcode.append('(yGrid)')
                for line in range(1, lines_y+1):
                    newy = (float(self.YPocketCenter.get())-(float(self.PocketY.get())/2.0)
                            ) + line*(float(self.PocketY.get())/(lines_y+1))
                    if self.grid_border.get() == 1:
                        newx_center_offset = (
                            float(self.PocketX.get())/2.0)-float(self.border.get())
                        x_mill_length = float(
                            self.PocketX.get())-(2*float(self.border.get()))
                    else:
                        newx_center_offset = float(self.PocketX.get())/2.0
                        x_mill_length = float(self.PocketX.get())
                    # zig zag
                    if line % 2 == 0:
                        newx = float(self.XPocketCenter.get()) + \
                            newx_center_offset
                        self.gcode.append('G0 X%.3f Y%.3f' % (newx, newy))
                        self.gcode.append('G0 Z%.3f' %
                                          (float(self.RapidZ.get())))
                        self.gcode.append('G1 Z%.3f F%.2f' % (
                            float(self.FinalDepth.get()), float(self.FeedspeedZVar.get())))
            ...
            if self.spider_grid.get() == "yes":
                ...
                for line in range(angel_lines):
                    ...
                    if 135 <= line_angel < 135 + 180:
                        self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                            150, 150, 150+line_x/Scale, 150+line_y/Scale, fill="red"))
                        self.gcode.append('G0 X%.3f Y%.3f' %
                                          (centerx, centery))
                        self.Go_down()
                        self.gcode.append('G1 X%.3f Y%.3f F%.2f' % (
                            centerx+line_x, centery+line_y, float(self.FeedspeedVar.get())))
                        self.Go_up()
                    else:
                        self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                            150, 150, 150-line_x/Scale, 150-line_y/Scale, fill="red"))
...
root = tk.Tk()
app = APP(master=root)
app.master.title('Grid.py by Sammel Lothar' + version)
app.mainloop()
