...
Version 1.6.0 - 2025-01-12 - Andrew Fernie
...
class APP (Frame):
    ...
    def createWidgets(self):
        ...
        self.PartLength = Entry(
            self, width=10, textvariable=self.PartLengthVar)
        ...
        self.TotalToRemove = Entry(
            self, width=10, textvariable=self.TotalToRemoveVar)
    ...
    def GenCode(self):
        ...
        if len(self.DepthOfCutVar.get()) > 0:
            ...
            self.NumOfZSteps = int(
                self.FToD(self.TotalToRemoveVar.get()) / self.Z_Step)
            if self.Z_Total % self.Z_Step > 0:
                self.NumOfZSteps = self.NumOfZSteps + 1
        else:
            self.Z_Step = 0
            self.NumOfZSteps = 1
        self.NumOfYSteps = int(
            ceil(self.FToD(self.PartWidthVar.get())/self.Y_StepOver))
        self.Z_Position = 0
    ...
    def LoadPrefs(self):
        self.NcDir = self.GetIniData(
            'face.ini', 'Directories', 'NcFiles', os.path.expanduser("~"))
        self.FeedrateVar.set(self.GetIniData(
            'face.ini', 'MillingPara', 'Feedrate', '1000'))
        self.DepthOfCutVar.set(self.GetIniData(
            'face.ini', 'MillingPara', 'DepthOfCut', '3'))
        self.ToolDiameterVar.set(self.GetIniData(
            'face.ini', 'MillingPara', 'ToolDiameter', '10'))
        self.SpindleRPMVar.set(self.GetIniData(
            'face.ini', 'MillingPara', 'SpindleRPM', '9000'))
        self.StepOverVar.set(self.GetIniData(
            'face.ini', 'MillingPara', 'StepOver', '50'))
        self.LeadinVar.set(self.GetIniData(
            'face.ini', 'MillingPara', 'Leadin'))
        self.UnitVar.set(
            int(self.GetIniData('face.ini', 'MillingPara', 'UnitVar', '2')))
        self.HomeVar.set(
            int(self.GetIniData('face.ini', 'MillingPara', 'HomeVar', '4')))
        self.SafeZVar.set(self.GetIniData(
            'face.ini', 'MillingPara', 'SafeZ', '10.0'))
        self.PartLengthVar.set(self.GetIniData('face.ini', 'Part', 'X'))
        self.PartWidthVar.set(self.GetIniData('face.ini', 'Part', 'Y'))
        self.TotalToRemoveVar.set(self.GetIniData(
            'face.ini', 'Part', 'TotalToRemove'))
