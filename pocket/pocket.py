...
class APP (Frame):
    ...
    def createWidgets(self):
        ...
        self.ShapeVar.set(0)
        Radiobutton(self.EntryFrame, text='Circle', variable=self.ShapeVar, value=0,
                    command=self.DoIt, indicatoron=0, width=5,).grid(row=self.rownumber+1, column=0, sticky=W)
        Radiobutton(self.EntryFrame, text=' Rect ',   variable=self.ShapeVar, value=2,
                    command=self.DoIt, indicatoron=0, width=5,).grid(row=self.rownumber+2, column=0, sticky=W)

        self.stPS = Label(self.EntryFrame, text='Path', width=7)
        self.stPS.grid(row=self.rownumber, column=0, sticky=E)
        self.PathVar = IntVar()
        self.PathVar.set(0)
        Radiobutton(self.EntryFrame, text='Paralell', variable=self.PathVar, value=0,
                    command=self.DoIt, indicatoron=0, width=7,).grid(row=self.rownumber+1, column=0, sticky=E)
        Radiobutton(self.EntryFrame, text=' Spiral ', variable=self.PathVar, value=1,
                    command=self.DoIt, indicatoron=0, width=7,).grid(row=self.rownumber+2, column=0, sticky=E)
        ...
        Radiobutton(self.EntryFrame, text='Zero', variable=self.MovmentVar, value=0, command=self.DoIt,
                    indicatoron=0, width=7,).grid(row=self.rownumber+1, column=1, sticky=W, padx=10)
        ...
        self.DirVar.set('CCW')
        Radiobutton(self.EntryFrame, text='CCW', variable=self.DirVar, value='CCW',
                    command=self.DoIt, indicatoron=0, width=7,).grid(row=self.rownumber+1, column=1, sticky=N)
        Radiobutton(self.EntryFrame, text='CW', variable=self.DirVar, value='CW', command=self.DoIt,
                    indicatoron=0, width=7,).grid(row=self.rownumber+2, column=1, sticky=N)
        ...
        self.PreambleVar.set(
            'G17 G21 G90 G64 P0.01%s M3 S3000 M7' % (toolChange))
        self.Preamble = Entry(
        ...
        self.ToolDiameter = Entry(
            self.EntryFrame, textvariable=self.ToolDiameterVar, width=8)
        ...
        self.ToolNumber = Spinbox(self.EntryFrame, textvariable=self.ToolNumberVar,
                                  from_=0, to=99, width=4, command=self.ChangeTool)
        self.ToolNumber.grid(row=self.rownumber, column=1, sticky=W)

        self.st10ab = Label(
            self.EntryFrame, text='             ToolOffsetNumber (D)')
        self.st10ab.grid(row=self.rownumber, column=1, sticky=N)
        self.ToolOffsetNumberVar = IntVar()
        self.ToolOffsetNumberVar.set('1')
        self.ToolOffsetNumber = Entry(
            self.EntryFrame, textvariable=self.ToolOffsetNumberVar, width=3)
        ...
        self.Finish = IntVar()
        self.Finish.set(0)
        Checkbutton(self.EntryFrame, text='Finish Contour', variable=self.Finish, onvalue=1,
                    offvalue=0, command=self.DoIt).grid(row=self.rownumber, column=1, sticky=E)
        ...
        self.OffsetXY = Entry(
            self.EntryFrame, textvariable=self.OffsetXYVar, width=5)
        ...
        self.OffsetZ = Entry(
            self.EntryFrame, textvariable=self.OffsetZVar, width=5)
        ...
        self.ZStepover = Entry(
            self.EntryFrame, textvariable=self.ZStepoverVar, width=8)
    ...
    def Change_Units(self):
        if (self.UnitVar.get() == 0):
            # Inch
            self.PreambleVar.set(
                'G17 G20 G90 G64 P0.001%s M3 S3000 M7' % (toolChange))
        else:
            # MM
            self.PreambleVar.set(
                'G17 G21 G90 G64 P0.01%s M3 S3000 M7' % (toolChange))
    ...
    def DoIt(self):
        ...
        self.gcode.append('G0 Z%.4f ' %
                          (float(self.SafeZVar.get())))  # G0 Zsafe
        self.gcode.append('G0 X%.4f Y%.4f ' % (float(self.XPocketCenterVar.get()), float(
        ...
        if self.ShapeVar.get() == 0:  # circel
            ...
            if self.PathVar.get() == 0:  # cricel parallell path
                if self.Finish.get() == 1:
                    numberofcircels = int(
                        (PocketRadius-ToolDiameterRad)/Spacing)
                else:
                    numberofcircels = int(
                        (PocketRadius-ToolDiameterRad-MaterialallowanceXY)/Spacing)
                canvasoldX = 150
                canvasoldY = 150
                for cir in range(numberofcircels, 1, -1):
                    ...
                    self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                        150, 150, canvasnewY, 150, fill='red'))
                    self.CanvasDrawings.append(self.PreviewCanvas.create_oval(
                        PocketRadius,
                        PocketRadius,
                        300-PocketRadius,
                ...
                # variable neww loadet after change from canvas
                PocketRadius = float(self.PocketXVar.get())/2.0
                self.gcode.append('(Parallel ZeroPath Circular Pocket )')
                self.gcode.append(
                    'G1 Z0.00 F%.2f (Move to Z Zero for incremental Z pathes )' % speedZ)
                centerx = float(self.XPocketCenter.get())
                ...
                while tempz < 0:
                    ...
                    if firstRun:
                        ...
                        gword = 90
                    if tempz < stepoverz:
                        self.gcode.append('G1 G%d Z%.3f F%.2f ' %
                                          (gword, stepoverz, speedZ))
                    else:
                        self.gcode.append('G1 G%d Z%.3f F%.2f ' %
                                          (gword, tempz, speedZ))
                    tempz = tempz-stepoverz
                    for cir in range(1, numberofcircels):
                        self.gcode.append('G1 G90 X%.3f F%.2f ' % (
                            (centerx+(cir*Spacing)), speedXY))
                    ...
                    if self.Finish.get() == 1:
                        ...
                        self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                            150, 150, 150+corner, 150, fill='red'))
                        self.CanvasDrawings.append(self.PreviewCanvas.create_oval(
                            150-corner, 150-corner, 150+corner, 150+corner, outline='Red'))
                        # Gcode finish
                        self.gcode.append('G1 G90 X%.3f' % (
                            (centerx+PocketRadius-ToolDiameterRad)))
                        if self.DirVar.get() == 'CCW':
                            self.gcode.append('G3 I-%.3f' %
                                              (PocketRadius-ToolDiameterRad))
                        else:
                            self.gcode.append('G2 I-%.3f' %
                                              (PocketRadius-ToolDiameterRad))
                    ...
                    if (tempz >= 0):
                        self.gcode.append('G0 Z%.4f ' % (
                            float(self.SafeZVar.get())))  # G0 Zsafe
                    self.gcode.append('G1 X%.4f Y%.4f ' % (float(
                        self.XPocketCenterVar.get()), float(self.YPocketCenterVar.get())))  # Center
                self.gcode.append('G0 Z%.4f ' %
                                  (float(self.SafeZVar.get())))  # G0 Zsafe
            else:  # cirel_spiral
                ...
                self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                    150, 150, 150+(Spacing/Scale), 150, fill='red'))
                for arc_curve in range(1, numberofcircels+1):
                    if arc_curve % 2 == 0:  # geradezahl
                        if arc_curve == 2:
                            # canvas_new_Y =150-((arc_curve*(versatz/2.0))/scale)
                            self.CanvasDrawings.append(self.PreviewCanvas.create_arc(
                                canvas_new_Y, canvas_new_Y, 300-canvas_new_Y+vsh, 300-canvas_new_Y, start=180, extent=180, outline='red', style=ARC))
                        else:
                            canvas_new_Y = 150 - \
                                ((arc_curve*(Spacing/2.0))/Scale)
                            self.CanvasDrawings.append(self.PreviewCanvas.create_arc(
                                canvas_new_Y+vsh, canvas_new_Y, 300-canvas_new_Y+vsh, 300-canvas_new_Y, start=180, extent=180, outline='red', style=ARC))
                    else:
                        if arc_curve == 1:
                            canvas_new_Y = 150-((arc_curve*Spacing)/Scale)
                            self.CanvasDrawings.append(self.PreviewCanvas.create_arc(
                                canvas_new_Y, canvas_new_Y, 300-canvas_new_Y, 300-canvas_new_Y, start=0, extent=180, outline='red', style=ARC))
                        else:
                            canvas_new_Y = 150 - \
                                ((arc_curve*(Spacing/2.0))/Scale)
                            self.CanvasDrawings.append(self.PreviewCanvas.create_arc(
                    ...
                    self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                        150, 150, 150+Radius-(vsh/2), 150, fill='red'))
                self.CanvasDrawings.append(self.PreviewCanvas.create_text(
                    (150, 280), text="Grafic may differ to G_code on some Enterys"))
                if self.Finish.get() == 1:
                    Radius = (PocketRadius-ToolDiameterRad)/Scale
                    self.CanvasDrawings.append(self.PreviewCanvas.create_oval(
                        150-Radius+(vsh/2),
                ...
                while tempz < 0:
                    ...
                    cycle = 1
                    self.gcode.append('G1 G90 X%.3f F%.2f ' %
                                      ((centerx+tempr), speedXY))
                    while cycle < cycels:
                        self.gcode.append('G3 X%.3f I-%.3f ' %
                                          (centerx-(cycle*Spacing), (tempI*Spacing)))
                        cycle = cycle+1
                        tempI = tempI+0.5
                        self.gcode.append('G3 X%.3f I%.3f ' % (
                            centerx+(cycle*Spacing), (tempI*Spacing)))
                        tempI = tempI+0.5
                    PocketRadius = float(self.PocketXVar.get())/2.0
                    newI = ((centerx+(cycle*Spacing)) -
                            (centerx-PocketRadius+ToolDiameterRad))/2.0
                    self.gcode.append(
        ...
        if self.ShapeVar.get() == 2:      # rectangle
            ...
            if self.PathVar.get() == 0:  # Parallel
                ...
                if pocket_length_x > pocket_length_y:
                    rec_cycels = int(
                        (pocket_length_y-ToolDiameter-offsetxy)/Spacing/2.0)
                else:
                    rec_cycels = int(
                        (pocket_length_x-ToolDiameter-offsetxy)/Spacing/2.0)
                first_length_x = (
                    (pocket_length_x-ToolDiameter)/2.0)-(rec_cycels*Spacing)
                first_length_y = (
                    (pocket_length_y-ToolDiameter)/2.0)-(rec_cycels*Spacing)
                self.gcode.append(
                    'G1 G90 Z0.00 F%.2f (Move to Z Zero for incremental Z pathes )' % speedZ)
                while tempz < 0:
                    gword = 91
                    if firstRun:
                        firstRun = False
                    ...
                    while while_value < rec_cycels:
                        canvas_new_X = (
                            first_length_x+(while_value*Spacing))/Scale
                        canvas_new_Y = (
                            first_length_y+(while_value*Spacing))/Scale
                        self.CanvasDrawings.append(self.PreviewCanvas.create_rectangle(
                            150-canvas_new_X, 150-canvas_new_Y, 150+canvas_new_X, 150+canvas_new_Y, outline='red'))
                        self.gcode.append('G1 G90 X%.3f Y%.3f F%.2f ' % (centerx+first_length_x+(
                            while_value*Spacing), centery+first_length_y+(while_value*Spacing), speedXY))
                        self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                            canvasoldX, canvasoldY, 150+canvas_new_X, 150-canvas_new_Y, fill='red'))
                        canvasoldX = 150+canvas_new_X
                        canvasoldY = 150-canvas_new_Y
                        self.gcode.append('G1 X%.3f ' % (
                            centerx-first_length_x-(while_value*Spacing)))
                    ...
                    if self.Finish.get() == 1:
                        self.gcode.append('G1 G90 X%.3f Y%.3f ' % (
                            (centerx+(pocket_length_x/2.0)-ToolDiameterRad), (centery+(pocket_length_y/2.0)-ToolDiameterRad)))
                    ...
                    if (tempz >= 0):
                        self.gcode.append('G0 Z%.4f ' % (
                            float(self.SafeZVar.get())))  # G0 Zsafe
                    self.gcode.append('G1 G90 X%.4f Y%.4f ' %
                                      (centerx, centery))  # Center
                self.gcode.append('G0 Z%.4f ' %
                                  (float(self.SafeZVar.get())))  # G0 Zsafe
            ...
            # ----------
                ...
                if pocket_length_x > pocket_length_y:
                    rec_cycels = int(
                        (pocket_length_x-ToolDiameter)/Spacing/2.0)
                else:
                    rec_cycels = int(
                        (pocket_length_y-ToolDiameter)/Spacing/2.0)
                ...
                while tempz < 0:
                    ...
                    while cycle < rec_cycels:
                        self.gcode.append('G1 G91 X%.3f F%.2f ' %
                                          (tempx, speedXY))
                        canvas_new_X = canvasoldX+(tempx/Scale)
                        self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                            canvasoldX, canvasoldY, canvas_new_X, canvasoldY, fill='red'))
                        canvasoldX = canvas_new_X
                        self.gcode.append('G1 Y%.3f ' % (tempy))
                        canvas_new_Y = canvasoldY-(tempy/Scale)
                        self.CanvasDrawings.append(self.PreviewCanvas.create_line(
                            canvasoldX, canvasoldY, canvasoldX, canvas_new_Y, fill='red'))
        ...
        self.CanvasDrawings.append(
            self.PreviewCanvas.create_oval(10, 275, 25, 290, fill='Blue'))
        self.CanvasDrawings.append(
            self.PreviewCanvas.create_text((20, 270), text="Tool"))
...
root = tk.Tk()
app = APP(master=root)
app.master.title('Pocket.py by Sammel Lothar V' + version)
app.mainloop()
