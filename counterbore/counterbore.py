...
"""
    ...
    Version 1.4.0 - 2025-01-13 - Andrew Fernie
...
class APP (Frame):
    ...
    def rb3Change(self):
        self.tmp = self.var3.get()
        self.ms = self.mSHCS[self.tmp]
    ...
    def createWidgets(self):
        self.title = Label(
            self, text='Socket Head Cap Screw Counterbore G-Code Generator')
        ...
        for text, value in self.SHCS1:
            self.rb1 = Radiobutton(self, text=text, value=value, variable=self.var1,
                                   font="monospace 10", command=self.rb1Change)
        ...
        for text, value in self.SHCS2:
            self.rb2 = Radiobutton(self, text=text, value=value, variable=self.var2,
                                   font="monospace 10", command=self.rb2Change)
        ...
        for text, value in self.SHCS3:
            self.rb3 = Radiobutton(self, text=text, value=value, variable=self.var3,
                                   font="monospace 10", command=self.rb3Change)
        ...
        self.HoleDiameterE = Entry(
            self, width=10, textvariable=self.HoleDiameterVar)
        ...
        self.ClearanceHeightE = Entry(
            self, width=10, textvariable=self.ClearanceHeightVar)
        ...
        self.MaterialTopE = Entry(
            self, width=10, textvariable=self.MaterialTopVar)
        ...
        self.StartHeightE = Entry(
            self, width=10, textvariable=self.StartHeightVar)
        ...
        self.SpiralDepthE = Entry(
            self, width=10, textvariable=self.SpiralDepthVar)
        ...
        for text, value in self.CutDirection:
            self.rb4 = Radiobutton(
                self, text=text, value=value, variable=self.CutDirectionVar)
        ...
        self.GenerateBtn = Button(
            self, text="Generate", command=self.GeneratePath, width=7)
        ...
        self.ClearGCode = Button(
            self, text='Clear G Code Window', command=self.ClearGCode)
        self.ClearGCode.grid(row=22, column=6, sticky=W)

        self.quitButton = Button(
            self, text="Cancel", command=self.quit, width=7)
    ...
    def GeneratePath(self):
        ...
        # Chicken Checking
        toolDiameter = self.ToolDiameterVar.get()
        if len(self.ToolDiameterVar.get()) == 0:
            messagebox.showwarning(
                'Entry Missing', 'Please Enter a Tool Diameter!')
            return
        ...
        if len(self.SpiralDepthVar.get()) == 0:
            ...
            self.NumberOfSpirals = int(
                ceil(self.CutDepth/float(self.SpiralDepthE.get())))
        ...
        for n in range(0, int(self.CordList.size())):
            ...
            if self.ToolDiameter <= self.HoleRadius:
                ...
                for n in range(0, self.NumberOfCuts):
                    ...
                    self.XMinus = self.XCenter - \
                        (self.StepOver + (self.ArcCenterOffset*2))
                    self.YMinus = self.YCenter - \
                        (self.StepOver + (self.ArcCenterOffset*4))
                    self.XPlus = self.XCenter + \
                        (self.StepOver + (self.ArcCenterOffset*6))
                ...
                for n in range(0, self.NumberOfCuts):
                    self.g_code.insert(END, 'G3 X%.4f Y%.4f Z%.4f J%.4f\n'
                                       % (self.XCenter, self.StartPositionY, -(self.NextZPosition), self.ArcRadius))
...
root = tk.Tk()
app = APP(master=root)
app.master.title('Counterbore' + version)
app.mainloop()
