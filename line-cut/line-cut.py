...
class APP (Frame):
    ...
    def LoadPrefs(self):
        self.NcDir = self.GetIniData(
            'line-cut.ini', 'Directories', 'NcFiles', os.path.expanduser("~"))
        self.FeedrateVar.set(self.GetIniData(
            'line-cut.ini', 'MillingPara', 'Feedrate', '1000'))
        self.DepthOfCutVar.set(self.GetIniData(
            'line-cut.ini', 'MillingPara', 'DepthOfCut', '3'))
        self.ToolDiameterVar.set(self.GetIniData(
            'line-cut.ini', 'MillingPara', 'ToolDiameter', '10'))
        self.SpindleRPMVar.set(self.GetIniData(
            'line-cut.ini', 'MillingPara', 'SpindleRPM', '9000'))
        self.LeadinVar.set(self.GetIniData(
            'line-cut.ini', 'MillingPara', 'Leadin'))
        self.UnitVar.set(
            int(self.GetIniData('line-cut.ini', 'MillingPara', 'UnitVar', '2')))
        self.HomeVar.set(
            int(self.GetIniData('line-cut.ini', 'MillingPara', 'HomeVar', '4')))
        self.CutAlongVar.set(
            int(self.GetIniData('line-cut.ini', 'MillingPara', 'CutAlongVar', '1')))
        self.SafeZVar.set(self.GetIniData(
            'line-cut.ini', 'MillingPara', 'SafeZ', '10.0'))
        self.CutLengthVar.set(self.GetIniData('line-cut.ini', 'Part', 'X'))
        self.TotalToRemoveVar.set(self.GetIniData(
            'cut.ini', 'Part', 'TotalToRemove'))
