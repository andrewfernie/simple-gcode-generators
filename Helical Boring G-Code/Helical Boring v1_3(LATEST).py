...
"""
	...
	Version 1.4.0 - 2025-01-14 - Andrew Fernie
...
class APP (Frame):
    ...
    def createwidgets(self):
        ...
        self.finish = IntVar()
        self.finishB = Checkbutton(
            self, text='Finish Pass', variable=self.finish, onvalue=1, offvalue=0)
        ...
        self.finishpassE = Entry(
            self, textvariable=self.finishpass, width=10)
        self.finishpassE.grid(row=3, column=1)

        self.spindle = IntVar()
        self.spindleB = Checkbutton(
            self, text='Turn on Spindle', variable=self.spindle, onvalue=1, offvalue=0)
        self.spindleB.grid(row=4, column=0, columnspan=2)

        self.coolant = IntVar()
        self.coolantB = Checkbutton(
            self, text='Coolant', variable=self.coolant, onvalue=1, offvalue=0)
        ...
        self.WOCtype = IntVar()
        self.WOCtypes = [('W.O.C. as % of Tool Diameter',
                          1), ('W.O.C. as Decimal', 2)]

        for text, value in self.WOCtypes:
            self.WOCtypeB = Radiobutton(
                self, text=text, value=value, variable=self.WOCtype)
        ...
        self.CreateB = Button(self, text='Create G-Code',
                              width=20, command=self.createCODE)
        ...
        self.textout = Text(self, width=50, height=40,
    ...
    def createCODE(self):
        ...
        if not self.h_N:
            ...
            self.textout.insert(
                INSERT, 'UNEQUAL NUMBER OF COORDINATES GIVEN')
        ...
        while self.coord <= self.h_n:
            ...
            exec('self.circleCode(self.c_x%s,self.c_y%s,self.c_z%s)' %
    ...
    def circleCode(self, c_x, c_y, c_z):
        ...
        if self.c_z >= self.r_z:
            ...
            self.textout.delete('1.0', END)
            self.textout.insert(
                INSERT, 'RAPID Z HEIGHT BENEATH Z CENTERPOINT')
        ...
        # RAPID X Y
        self.textout.insert(INSERT, '\nG00 X%s Y%s' %
                            ((round(self.s_x, 4)), (round(self.s_y))))

        # Feed to Z

        self.textout.insert(INSERT, '\nG01 Z%s F%s' %
                            ((round(self.s_z, 4)), (round(self.fr, 4))))
        ...
        while self.i_z >= self.f_z:
            ...
            while self.s_nz >= self.i_z:
                ...
                self.textout.insert(INSERT, '\nG03 I%s Z%s' % (
                    (round(self.e_r, 4)), (round(self.s_nz, 4))))

                self.s_nz -= self.s_dz

            self.textout.insert(INSERT, '\nG03 I%s Z%s' % (
                (round(self.e_r, 4)), (round(self.i_z, 4))))
            ...
            while self.n_r <= self.r_r:
                ...
                self.textout.insert(INSERT, '\nG01 X%s Y%s' % (
                    (round(self.n_x, 4)), (round(self.s_y, 4))))
                self.textout.insert(INSERT, '\nG03 I%s' %
                                    ((round(self.n_r, 4))))
            ...
            self.textout.insert(INSERT, '\nG01 X%s Y%s' % (
                (round(self.r_x, 4)), (round(self.s_y, 4))))
        ...
        if self.finish.get() == 1:
            ...
            self.textout.insert(INSERT, '\nG01 X%s Y%s' % (
                (round(self.f_x, 4)), (round(self.s_y, 4))))
...
root = tk.Tk()
app = APP(master=root)
app.master.title('Helical Boring G-Code ' + version)
app.mainloop()
