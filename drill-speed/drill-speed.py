...
class APP (Frame):
    ...
    def createFrames(self):
        self.f1 = Frame(self, width=150, height=235, bd=5, relief=RIDGE)
        self.f1.grid(row=0, column=0)
        self.f1.grid_propagate(0)
        self.f2 = Frame(self, width=150, height=235, bd=5, relief=RIDGE)
        self.f2.grid(row=0, column=1)
        self.f2.grid_propagate(0)
        self.f3 = Frame(self, width=150, height=235, bd=5, relief=RIDGE)
        self.f3.grid(row=0, column=2)
        self.f3.grid_propagate(0)
        self.f4 = Frame(self, width=450, height=110, bd=5, relief=RIDGE)
        self.f4.grid(row=1, column=0, columnspan=3)
        self.f4.grid_propagate(0)

    def f1rb1Change(self):
    ...
    def createWidgets(self):
        ...
        for text, value in self.Material:
            self.f1rb1 = Radiobutton(
                self.f1, text=text, value=value, variable=self.f1rb1Var, command=self.f1rb1Change)
        ...
        # Force update of drill list data
        self.f1rb1Var.set(1)
        self.f1rb1Change()
...
root = tk.Tk()
app = APP(master=root)
app.master.title('Drilling Speeds & Feeds ' + version)
app.mainloop()
